...
A release ends at a tagged GitHub Release with the easy artifact the spec chose attached: a versioned zip or tarball by default, a self-extracting exe or an MSI where the spec asked for one, a single binary for a CLI. Forge builds it, smoke-tests it outside the build tree, and warns when a binary is unsigned; signing, notarization, app stores, package registries, and auto-update are named as yours rather than done quietly, and a hosted service gets a line saying why no artifact applies.
